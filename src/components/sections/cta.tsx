    ...
    >
      <div className="grid items-center gap-10 lg:grid-cols-[2fr_3fr] lg:gap-0">
        <div className="relative w-full px-3 sm:px-4 lg:min-h-[520px] lg:pl-[max(0.75rem,calc((100vw-80rem)/2+0.75rem))] lg:pr-4">
          <div className="relative mx-auto aspect-[885/748] w-full max-w-[480px] sm:max-w-[520px] lg:absolute lg:inset-y-0 lg:left-[max(0.75rem,calc((100vw-80rem)/2+0.75rem))] lg:right-auto lg:mx-0 lg:ml-6 lg:aspect-auto lg:h-full lg:max-w-none lg:w-[calc(100%-1.5rem)]">
            <Image
        ...
        <div className="space-y-6 px-3 sm:px-4 lg:ml-6 lg:pl-4 lg:pr-[max(0.75rem,calc((100vw-80rem)/2+0.75rem))]">
          <h2 className="border-0 bg-transparent font-display text-[clamp(1.25rem,3.5vw,3.125rem)] font-normal leading-[1.15] text-white shadow-none">
            {ctaContent.headline}
          </h2>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5" aria-hidden>
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  className="h-7 w-7 fill-[#00ff00] text-[#00ff00] sm:h-8 sm:w-8"
                  strokeWidth={0}
                />
              ))}
            </div>
      ...
      <Container className="mt-8 max-w-[min(92%,90rem)] px-3 sm:mt-10 sm:px-4 md:mt-16">
        <div
          className={cn(
            "flex flex-col gap-5 rounded-2xl px-4 py-5 sm:gap-6 sm:px-6 sm:py-6 md:px-8 md:py-8 lg:flex-row lg:items-center lg:justify-between",
        ...
        >
          <div className="flex min-w-0 items-start gap-3 sm:items-center sm:gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-balance text-lg font-semibold leading-snug text-white sm:text-xl md:text-2xl">
                {ctaContent.bar.title}
              </p>
            </div>
          </div>

          <StoreButtons size="small" className="w-full shrink-0 lg:hidden" />
